...
## Connecting RStudio & Github for the *first* time
...
1) Open RStudio. From the RStudio toolbar at the top, open Tools > Global Options
![](img/1_git.png)
2) Click on Git/SVN. If the box that says "Enable version control interface for RStudio projects" is not checked, check the box. Then, click "Browse" next to "Git executable" and enter the filepath to your git (which you already should have installed, in step 5!) To find your git filepath, go to your Terminal and type  which git then hit enter.
![](img/2_git.png)
3) Click on "Create SSH Key"
![](img/3_git.png)
4) Click Create, without changing the defaults. You can leave Passphrase blank.

5) Highlight and copy the filepath listed under "The SSH key will be created at:" using Ctrl C or Command C

6) Next click "view public key" and copy all of the text in the pop-up window using using Ctrl C or Command C to get the public key contents in your clipboard. 
![](img/6_git.png)

7) Go to your Github Account > profile photo > Settings as shown below
![](img/7_git.png)
8) Follow these steps: (your public key is now in your clipboard, so when you get to "paste your public key" you can just hit Ctrl V or Command V.
![](img/8_git.png)
9) Assuming you've created your own repository already, go to https://github.com/orgs/UCD-R-DAVIS/repositories and click on the one with your username on the end. (If you haven't made a repository yet, make sure you've filled out the survey https://forms.gle/YahVutQ6Ds1CpHFP6 and followed the instructions)

10) The next step is to clone your repository. To start this process, click the Green "Code" button then click the "SSH" tab. Copy the link using the Screenshot 2023-09-28 at 10.30.16 AM.png icon
![](img/10_git.png)

11) Now we're ready to make a local version of our repository on our desktop. Open RStudio and click on New Project. 
![](img/11_git.png)
12) Click on "Version Control".
![](img/12_git.png)
13) Click on "Git"
![](img/13_git.png)
14) Paste the link from your clipboard (using Ctrl V or Command V) where it says "Repository URL". The "Project directory name" is what you want your R-DAVIS folder to be called. If you hit Tab, the Project directory name will automatically create a name for you. This is generally preferred so you can keep track of things more easily. Lastly, decide where you want your R projects to live on your computer, and navigate there using "Browse". For example, I've created a folder called "R_Projects". (Note the underline -- it's a good idea to not have any spaces in your filenames or filepaths.) For the rest of this class, everything we do will be in this folder. 

As a forewarning, sometimes people have had trouble using MyCloud or other cloud-based storage for their R folders, so we recommend storing locally on your desktop. 
![](img/14_git.png)
15) Click "Create Project" -- and you're done! 


Now you have your very own corner of the internet where all of your work for this class will live.


## Already have an SSH key set up? Here's how to connect a Github respository and RStudio
...
Now we'll create an RStudio Project and link our Github repository (following [*Section 13.3* of `happygitwithR`](http://happygitwithr.com/rstudio-git-github.html#clone-the-new-github-repository-to-your-computer-via-rstudio)).
  ...
  - In the “**Repository URL**” paste the URL of your new GitHub repository. If you hit **`Tab`**, the *Project directory name* will automatically populate with the exact name of the repository. This is generally preferred so you can keep track of things more easily. You should put this project into a subdirectory. In this example, I've created a **Desktop/R_DAVIS_2024** for this class. (Note the underline -- it's a good idea to not have any spaces in your filenames or filepaths.) For the rest of this class, everything we do will be in this folder. 
  
![](img/rstudio_git_repo_setup.png)

<br>

For the rest of your life, however, you can put GitHub repositories anywhere on your computer, but it's really useful to keep them well organized. A common way to do this would be a folder under your username called "R_Projects" or something like that.
