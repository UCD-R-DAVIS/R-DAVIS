...
7. Add trendlines to the plot to show the relationship between speed and SPM for each of the three time periods (hint: you might want to check out geom_smooth)

8. Does this relationship maintain or break down as Tyler gets tired? Make a plot (of your choosing) that shows SPM vs. speed by *lap*. Use the `timestamp` indicator to assign lap numbers, assuming that all laps on a given day correspond to the same run. Select only laps 1-3 (Tyler never runs more than three miles these days). Make a plot that shows SPM, speed, *and* lap number (pick a visualization that you think best shows these three variables).
...
local_url <- 'data/tyler_activity_laps_12-6.csv'
lap_dt <- read_csv(local_url)

running_laps <- lap_dt %>% 
  filter(sport == 'running') %>%
  filter(total_elapsed_time_s >= 60) %>%
  filter(minutes_per_mile < 10 & minutes_per_mile > 5) %>%
        mutate(form = case_when(year < 2024 ~ 'pre-2024',
                          month < 7 & year == 2024 ~ 'early-2024',
                          T ~ 'late-2024'))

ggplot(running_laps,aes(y = steps_per_minute,x = minutes_per_mile,color = form)) + 
  geom_point(alpha = 0.2,pch = 19) + 
  scale_x_reverse() + 
  theme_bw() + scale_color_brewer(type='qual',palette = 3) + 
  ggtitle("steps-per-minute vs. mile pace") +
  labs(xlab = 'mile pace (minutes-per-mile)',ylab = 'steps-per-minute') +
  geom_smooth(method = 'gam') 
library(lubridate)
late2024_laps <- running_laps |> 
...
ggplot(data = late2024_laps,aes(x=minutes_per_mile,y = steps_per_minute,colour = as.factor(lap))) + 
  scale_x_reverse() + 
  geom_point() +
  scale_color_brewer(type='qual',palette = 2,name = 'lap') +
