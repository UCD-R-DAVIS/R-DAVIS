...
BACKGROUND: For the midterm, you compared Tyler's old running data with recent data to analyze to see if there was any difference in strides-per-minute (SPM). Tyler's reform project started in January, 2024. On July 1, 2024, Tyler went to a follow-up appointment with the UCD Sports Medicine clinic, and they told him that has cadence was still too low, and that his form was perhaps *more* damaging than it had been. The technician gave Tyler some training cues, as well as the advice that "elite professional runners are at least 180 strides-per-minute, so aim for that". 
...
8. Does this relationship maintain or break down as Tyler gets tired? Make a plot (of your choosing) that shows SPM vs. speed by *lap*. Note that because you removed Tyler's walk breaks, the lap numbers are slightly off. To recode, arrange 
...
ggplot(running_laps) + 
  geom_point(aes(y = steps_per_minute,x = minutes_per_mile))
